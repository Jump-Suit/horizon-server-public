...
namespace Server.Medius.HTTPService
{
    sealed class CrudServerHandler : ChannelHandlerAdapter
    {
        ...
        static readonly IInternalLogger _logger = InternalLoggerFactory.GetInstance<CrudServerHandler>();
        static readonly TimeSpan _defaultTimeout = TimeSpan.FromMilliseconds(3000);

        private IInternalLogger Logger => _logger;
        ...
        {
            Logger.Info($"[Crud_Room_Manager] - Client - {ctx.Channel.RemoteAddress} Requested the HTTP/S Server with URL : {request.Uri}");
